...
typedef struct {
  ...
  pa_dram_t physical_page_number; // page number (PPN)
} tlb_entry_t;
...
static int tlb_l1_find_lru_entry() {
  ...
  for (int i = 1; i < TLB_L1_SIZE; i++) {
    if (!tlb_l1[i].valid) {
      return i; // Found an invalid (empty) entry — use it directly
    }
...
// Invalidate an L1 entry that matches given virtual page number (helper)
static void tlb_l1_invalidate_by_vpn(va_t virtual_page_number) {
  int idx = tlb_l1_find_entry(virtual_page_number);
  if (idx != -1) {
...
void tlb_invalidate(va_t virtual_page_number) {
  // Invalidate should incur both L1 and L2 TLB latencies (we account both)
  increment_time(TLB_L1_LATENCY_NS);
  increment_time(TLB_L2_LATENCY_NS);

  // Invalidate entry in L1 TLB
  tlb_l1_invalidate_by_vpn(virtual_page_number);

  // Invalidate entry in L2 TLB
  int index = tlb_l2_find_entry(virtual_page_number);
  if (index != -1) {
...
// Translate virtual address to physical address using TLBs + page table.
// Returns physical address (with offset).
pa_dram_t tlb_translate(va_t virtual_address, op_t op) {
  ...
  if (l1_index != -1) {
    // L1 TLB hit
    tlb_l1_hits++;
    // Update last access AFTER having accounted latencies so timestamp reflects real time
  ...
  // L1 TLB miss - need to check L2
  tlb_l1_misses++;

  // Account for L2 latency when checking L2
  increment_time(TLB_L2_LATENCY_NS);
  ...
  if (l2_index != -1) {
    ...
    tlb_l1[replace_index].dirty = false;

    // Update L1 TLB entry (keep L2 copy, inclusive)
  ...
  // L2 miss -> consult page table
  tlb_l2_misses++;

  // Get physical address from page table (assumed to return full physical address including offset)
  pa_dram_t pa = page_table_translate(virtual_address, op);
  pa_dram_t ppn = pa >> PAGE_SIZE_BITS;

  // Insert into L2 (inclusive cache). On L2 replacement, if we evict an entry we must
  // invalidate any L1 copies of that virtual page (to preserve inclusivity).
  ...
  // Return physical address constructed from ppn + offset for consistency
  return (ppn << PAGE_SIZE_BITS) | virtual_page_offset;
}
